...
export default function Smeloanshistory() {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      contentContainerStyle={{ padding: 20 }}>
       ...
       {agent.smeloans.length < 1 && (
          <View style={{ width: '100%', height: 250, backgroundColor: 'white', justifyContent: 'center', alignItems: 'center'}}>
            <Text>You haven't created any SME loans yet!</Text>  
          </View>
       )}

       {agent.smeloans.length > 0 && !loading && agent.smeloans.map((item, index) => (
          <View key={index.toString()} style={{ width: '100%', height: 150, backgroundColor: 'whitesmoke', flexDirection: 'row', marginBottom: 20, borderRadius: 5 }}>
            ...
            <View style={{ flex: 0.6, paddingHorizontal: 20, justifyContent: 'center' }}>
                <Text style={{ fontSize: 18, fontFamily: theme.fonts.RobotoRegular }}>{item.business_name}</Text>
            ...
            <View style={{ flex: 0.4, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 20 }}>
                <View style={{ width: '100%', height: 40, borderRadius: 5, borderWidth: 2, borderColor: Color(item.status), justifyContent: 'center', alignItems: 'center'}}>
                  <Text style={{ fontFamily: theme.fonts.PoppinsSemiBold, fontWeight: '500', color: Color(item.status) }}>{text(item.status)}</Text>
                </View>
            </View>
